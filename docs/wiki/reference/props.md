...
| `mode`            | `"in-place" \| "source" \| "preview" \| "split"` | `"source"` | Which view to show. `source`, `preview`, and `split` are complete. `in-place` is opt-in while its decoration canvas is built increment by increment ([ADR-004](../../journal/2026-09/2026-09-01_adr-004-in-place-decoration-canvas.md), [tracker](../../journal/2026-09/2026-09-01_in-place-canvas.md)) — today it renders headings only. `split` expects the root element to have a bounded height. The default becomes `in-place` once the canvas is finished ([ADR-002](../../journal/2026-09/2026-09-01_adr-002-editor-ux-and-customization.md) §1). |

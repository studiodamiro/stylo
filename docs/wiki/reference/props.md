...
| `mode`            | `"in-place" \| "source" \| "preview" \| "split"` | `"source"` | Which view to show. **Only `source` and `preview` are implemented today**; `in-place` and `split` warn once and fall back to `source`. The default becomes `in-place` once the decoration canvas lands ([ADR-002](../../journal/2026-09/2026-09-01_adr-002-editor-ux-and-customization.md)). |
